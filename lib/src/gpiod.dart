...
@immutable class _GlobalSignalEvent {
  ...
  factory _GlobalSignalEvent._fromList(List list) {
    return _GlobalSignalEvent(
      list[0] as int,
      SignalEvent._fromList(list.sublist(1))
    );
  }
...
@immutable class SignalEvent {
  ...
  factory SignalEvent._fromList(List list) {
    ...
    final time = DateTime.fromMicrosecondsSinceEpoch(s*1000000 + (ns/1000).round());
...
@immutable class _FlutterGpiodPlatformSide {
  static const _methodChannel = MethodChannel("plugins.flutter.io/gpiod", StandardMethodCodec());
  ...
  static Future<int> getLineHandle(int chipIndex, int lineIndex) async {
    return (await _methodChannel.invokeMethod("getLineHandle", <int>[chipIndex, lineIndex])) as int;
  }
  
  static Future<LineInfo> getLineInfo(int lineHandle) async {
    return LineInfo._fromMap(
      await _methodChannel.invokeMapMethod<String, dynamic>("getLineDetails", lineHandle)
    );
  }
  
  static Future<void> requestLine({int lineHandle,
                                   String consumer,
  ...
  static Future<void> reconfigureLine({int lineHandle,
                                       LineDirection direction,
                                       OutputMode outputMode,
                                       Bias bias,
                                       ActiveState activeState,
  ...
  static Future<bool> getLineValue(int lineHandle) async {
    return (await _methodChannel.invokeMethod("getLineValue", lineHandle)) as bool;
  }
  ...
  static Future<bool> supportsLineReconfiguration() async {
    return (await _methodChannel.invokeMethod("supportsLineReconfiguration")) as bool;
  }
}
  ...
  static Future<void> initialize() async {
    if (_instance == null) {
      final List<GpioChip> chips = List.unmodifiable(
        await Future.wait(
          List.generate(
            await _FlutterGpiodPlatformSide.getNumChips(),
...
@immutable class GpioChip {
  ...
  static Future<GpioChip> _fromIndex(int chipIndex) async {
    ...
    final lines = await Future.wait(
      List.generate(
        details['numLines'],
        (i) async => await GpioLine._fromHandle(await _FlutterGpiodPlatformSide.getLineHandle(chipIndex, i))
      )
    );
...
@immutable class LineInfo {
  ...
  const LineInfo._({this.name, this.consumer,
                         this.direction, this.outputMode,
                         this.bias, this.activeState,
                         this.isUsed, this.isRequested, this.isFree});

  factory LineInfo._fromMap(Map<String, dynamic> map) {
    return LineInfo._(
      name: map['name'] as String,
      consumer: map['consumer'] as String,
      direction: LineDirection.values.firstWhere(
        (v) => v.toString() == map['direction']
      ),
      outputMode: OutputMode.values.firstWhere(
        (v) => v.toString() == map['outputMode'],
        orElse: () => null
      ),
      bias: Bias.values.firstWhere(
        (v) => v.toString() == map['bias'],
        orElse: () => null
      ),
      activeState: ActiveState.values.firstWhere(
        (v) => v.toString() == map['activeState']
      ),
...
class GpioLine {
  ...
  void _assertNotWriteLocked() {
    if (_mutex.isWriteLocked) {
      throw StateError(
        "Action can't finish synchronously because "
  ...
  LineInfo get infoSync {
    ...
    if (!requested) {
      throw StateError(
        "Can't get line info because line "
        "is not requested."
      );
    }
  ...
  Future<void> requestInput({String consumer,
                             Bias bias,
  ...
  Future<void> requestOutput({String consumer,
  ...
  /// limitations.
  Future<void> reconfigureInput({Bias bias,
                                 ActiveState activeState = ActiveState.high}) {
    ArgumentError.checkNotNull(activeState, "activeState");
    _checkSupportsBiasValue(bias);
  ...
  /// This will throw a [UnsupportedError] if 
  /// [FlutterGpiod.supportsLineReconfiguration] is false.
  Future<void> reconfigureOutput({OutputMode outputMode = OutputMode.pushPull,
                                  Bias bias,
                                  ActiveState activeState = ActiveState.high,
  ...
  FutureOr<bool> getValue() {
    if (_mutex.isWriteLocked) {
      ...
      if (_info.direction == LineDirection.output) {
        ...
        return _synchronizedRead(() => _FlutterGpiodPlatformSide.getLineValue(_lineHandle));
      }
    }
  }
